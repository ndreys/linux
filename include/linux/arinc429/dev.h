...
struct arinc429_priv {
	struct arinc429_device_stats arinc429_stats;

	struct arinc429_clock clock;

	enum arinc429_state state;
	u32 ctrlmode;
	...
	int (*do_set_clock)(struct net_device *dev);
...
struct sk_buff *alloc_arinc429_skb(struct net_device *dev,
				struct arinc429_frame **cfd);
struct sk_buff *alloc_arinc429_err_skb(struct net_device *dev,
