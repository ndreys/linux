 ...
 * @loop: loopback for listeners on local ARINC429 sockets (recommended default!)
 *
...
int arinc429_send(struct sk_buff *skb, int loop)
{
	...
	/*
	 * Make sure the ARINC429 frame can pass the selected ARINC429 netdevice.
	 * As structs arinc429_frame and arinc429_frame are similar, we can provide
	 * ARINC429 FD frames to legacy ARINC429 drivers as long as the length is <= 8
	 */
	...
	if (loop) {
		 ...
		 * its own. Example: arinc429_raw sockopt ARINC429_RAW_RECV_OWN_MSGS
 ...
 * @dev: pointer to netdevice (NULL => subcribe from 'all' ARINC429 devices list)
 ...
 *
 *  The filter can be inverted (ARINC429_INV_FILTER bit set in arinc429_id) or it can
 *  filter for error message frames (ARINC429_ERR_FLAG bit set in mask).
 *
...
dev_err(&dev->dev, "Registering filter: label=%02x mask=%02x\n", label, mask);
		r->label   = label;
		r->mask    = mask;
		r->matches = 0;
		r->func    = func;
...
/**
 * arinc429_rx_unregister - unsubscribe ARINC429 frames from a specific interface
 * @dev: pointer to netdevice (NULL => unsubscribe from 'all' ARINC429 devices list)
			    ...
			    void (*func)(struct sk_buff *, void *), void *data)
{
	...
	rl = find_rcv_list(&label, &mask, inv, d);
dev_err(&dev->dev, "De-registering filter: label=%02x mask=%02x\n", label, mask);
			...
			struct packet_type *pt, struct net_device *orig_dev)
{
	...
	ret = WARN_ONCE(dev->type != ARPHRD_ARINC429 || skb->len != ARINC429_MTU,
...
static __init int arinc429_init(void)
{
	...
	rcv_cache = kmem_cache_create("arinc429_receiver", sizeof(struct receiver),
				      0, 0, NULL);
	if (!rcv_cache)
		return -ENOMEM;

	/* the statistics are updated every second (timer triggered) */
	timer_setup(&arinc429_stattimer, arinc429_stat_update, 0);
...
static __exit void arinc429_exit(void)
{
	...
	/* remove created dev_rcv_lists from still registered ARINC429 devices */
	rcu_read_lock();
