...
static void raw_rcv(struct sk_buff *oskb, void *data)
{
	...
	/* do not pass non-ARINC4292.0 frames to a legacy socket */
	if (!ro->fd_frames && oskb->len != ARINC429_MTU)
		return;
...
static int raw_getname(struct socket *sock, struct sockaddr *uaddr, int peer)
{
	struct sockaddr_arinc429 *addr = (struct sockaddr_arinc429 *)uaddr;
	struct sock *sk = sock->sk;
	struct raw_sock *ro = raw_sk(sk);
...
static int raw_sendmsg(struct socket *sock, struct msghdr *msg, size_t size)
{
	...
	if (msg->msg_name) {
		DECLARE_SOCKADDR(struct sockaddr_arinc429 *, addr, msg->msg_name);
...
static const struct proto_ops raw_ops = {
	...
	.getname       = raw_getname,
	.poll          = datagram_poll,
	.ioctl         = arinc429_ioctl,	/* use arinc429_ioctl() from af_arinc429.c */
...
static __init int raw_module_init(void)
{
	...
	if (err < 0)
		printk(KERN_ERR "arinc429: registration of raw protocol failed\n");

	pr_info("arinc429: raw: AF_ARINC429=%i\n", AF_ARINC429);
	pr_info("arinc429: raw: PF_ARINC429=%i\n", PF_ARINC429);
	pr_info("arinc429: raw: ARINC429_RAW=%i\n", ARINC429_RAW);
	pr_info("arinc429: raw: SOL_ARINC429_RAW=%i\n", SOL_ARINC429_RAW);
	pr_info("arinc429: raw: ARINC429_RAW_FILTER=%i\n", ARINC429_RAW_FILTER);
