			       ...
			       unsigned long count)
{
	...
	if (count > (ULONG_MAX / HZ)) {
		printk(KERN_ERR "arinc429: calc_rate: count exceeded! %ld\n",
		       count);
		return 99999999;
	}
...
void arinc429_stat_update(struct timer_list *unused)
{
	...
	if (arinc429_stats.rx_frames)
		arinc429_stats.total_rx_match_ratio = (arinc429_stats.matches * 100) /
			arinc429_stats.rx_frames;

	arinc429_stats.total_tx_rate = calc_rate(arinc429_stats.jiffies_init, j,
					    arinc429_stats.tx_frames);
	arinc429_stats.total_rx_rate = calc_rate(arinc429_stats.jiffies_init, j,
					    arinc429_stats.rx_frames);
	...
	arinc429_stats.current_tx_rate = calc_rate(0, HZ, arinc429_stats.tx_frames_delta);
	arinc429_stats.current_rx_rate = calc_rate(0, HZ, arinc429_stats.rx_frames_delta);
...
static void arinc429_print_rcvlist(struct seq_file *m, struct hlist_head *rx_list,
...
static int arinc429_reset_stats_proc_open(struct inode *inode, struct file *file)
{
...
static int arinc429_rcvlist_proc_show(struct seq_file *m, void *v)
{
	...
	for_each_netdev_rcu(&init_net, dev) {
		if (dev->type == ARPHRD_ARINC429 && dev->ml_priv)
			arinc429_rcvlist_proc_show_one(m, idx, dev, dev->ml_priv);
	}
...
void arinc429_init_proc(void)
{
	...
	pde_version     = proc_create(ARINC429_PROC_VERSION, 0644, arinc429_dir,
				      &arinc429_version_proc_fops);
	pde_stats       = proc_create(ARINC429_PROC_STATS, 0644, arinc429_dir,
				      &arinc429_stats_proc_fops);
	pde_reset_stats = proc_create(ARINC429_PROC_RESET_STATS, 0644, arinc429_dir,
				      &arinc429_reset_stats_proc_fops);
	pde_rcvlist_all = proc_create_data(ARINC429_PROC_RCVLIST_ALL, 0644, arinc429_dir,
					   &arinc429_rcvlist_proc_fops, (void *)RX_ALL);
	pde_rcvlist_fil = proc_create_data(ARINC429_PROC_RCVLIST_FIL, 0644, arinc429_dir,
					   &arinc429_rcvlist_proc_fops, (void *)RX_FIL);
	pde_rcvlist_inv = proc_create_data(ARINC429_PROC_RCVLIST_INV, 0644, arinc429_dir,
					   &arinc429_rcvlist_proc_fops, (void *)RX_INV);
}
