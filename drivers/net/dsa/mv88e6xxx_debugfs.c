...
static int _mv88e6xxx_phy_page_read(struct mv88e6xxx_priv_state *ps, int port, int page,
				    int reg)
{
	int ret;
...
static int mv88e6xxx_regs_show(struct seq_file *s, void *p)
{
	...
	for (reg = 0; reg < 32; reg++) {
		...
		ret = _mv88e6xxx_phy_page_read(ps, REG_FIBER_SERDES,
				    ...
				    size_t count, loff_t *ppos)
{
	...
	if (strcasecmp(name, "GLOBAL") == 0)
		ret = mv88e6xxx_reg_write(ps, REG_GLOBAL, reg, val);
	else if (strcasecmp(name, "GLOBAL2") == 0)
		ret = mv88e6xxx_reg_write(ps, REG_GLOBAL2, reg, val);
		...
		ret = mv88e6xxx_reg_write(ps, REG_PORT(port), reg, val);
	else
		return -EINVAL;
...
{
	struct mv88e6xxx_priv_state *ps = s->private;
	struct mv88e6xxx_priv_state *other_ps;
	struct dsa_switch *ds = ps->ds;
	...
	for (i = 0; i < DSA_MAX_SWITCHES; i++) {
		...
		/* Lets assume the other device is also a mv88e6xxx */
		other_ps = dst->ds[i]->priv;
		for (j = 0; j < other_ps->num_ports; ++j) {
			err = _mv88e6xxx_pvt_read(ps, i, j, &pvlan);
			if (err)
				goto unlock;

			seq_printf(s, "  %d   %2d   %03hhx ", i, j, pvlan);

			/* One column per internal output port */
			for (k = 0; k < ps->num_ports; ++k)
				seq_printf(s, "  %c",
					   pvlan & BIT(k) ? '*' : '-');
			seq_puts(s, "\n");
		}
				    ...
				    size_t count, loff_t *ppos)
{
	...
	char cmd[32];
	unsigned int dev, port, pvlan;
	int ret;

	if (copy_from_user(cmd, buf, sizeof(cmd)))
		return -EFAULT;

	if (sscanf(cmd, "%d %d %x", &dev, &port, &pvlan) != 3)
		return -EINVAL;

	if (dev >= DSA_MAX_SWITCHES || port >= DSA_MAX_PORTS || pvlan & ~mask)
		return -ERANGE;

	mutex_lock(&ps->smi_mutex);
	ret = _mv88e6xxx_pvt_write(ps, dev, port, pvlan);
	mutex_unlock(&ps->smi_mutex);
...
static void mv88e6xxx_init_debugfs(struct mv88e6xxx_priv_state *ps)
{
	...
	debugfs_create_file("atu", S_IRUGO, ps->dbgfs, ps,
			    &mv88e6xxx_atu_fops);
