				    ...
				    size_t count, loff_t *ppos)
{
	...
	if (strcasecmp(name, "GLOBAL") == 0)
		...
		ret = mv88e6xxx_reg_port_write(ps, port, reg, val);
	else
		ret = -EINVAL;
...
static int mv88e6xxx_state_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < ps->info->num_ports; ++i) {
		...
		ret = mv88e6xxx_reg_port_read(ps, i, PORT_CONTROL);
		if (ret < 0)
			goto unlock;
...
static int mv88e6xxx_8021q_mode_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < ps->info->num_ports; ++i) {
		...
		ret = mv88e6xxx_reg_port_read(ps, i, PORT_CONTROL_2);
		if (ret < 0)
			goto unlock;
...
static int mv88e6xxx_vlan_table_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < ps->info->num_ports; ++i) {
		...
		ret = mv88e6xxx_reg_port_read(ps, i, PORT_BASE_VLAN);
		if (ret < 0)
			goto unlock;
...
static void mv88e6xxx_init_debugfs(struct mv88e6xxx_priv_state *ps)
{
	...
	debugfs_create_file("name", S_IRUGO, ps->dbgfs, ps,
			    &mv88e6xxx_name_fops);
