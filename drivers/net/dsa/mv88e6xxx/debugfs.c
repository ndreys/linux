...
static int mv88e6xxx_regs_show(struct seq_file *s, void *p)
{
	...
	for (reg = 0; reg < 32; reg++) {
		...
		for (port = 0; port < chip->info->num_ports; ++port) {
			ret = mv88e6xxx_port_read(chip, port, reg, &val);
			if (ret < 0)
				goto unlock;
...
static int mv88e6xxx_state_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < chip->info->num_ports; ++i) {
		...
		ret = mv88e6xxx_port_read(chip, i, PORT_CONTROL, &val);
		if (ret < 0)
			goto unlock;
...
static int mv88e6xxx_8021q_mode_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < chip->info->num_ports; ++i) {
		...
		ret = mv88e6xxx_port_read(chip, i, PORT_CONTROL_2, &val);
		if (ret < 0)
			goto unlock;
...
static int mv88e6xxx_vlan_table_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < chip->info->num_ports; ++i) {
		...
		ret = mv88e6xxx_port_read(chip, i, PORT_BASE_VLAN, &val);
		if (ret < 0)
			goto unlock;

		/* One column per output port */
		for (j = 0; j < chip->info->num_ports; ++j)
			seq_printf(s, "  %c", val & BIT(j) ? '*' : '-');
		seq_puts(s, "\n");
	}
