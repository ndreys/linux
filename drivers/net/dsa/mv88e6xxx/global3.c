...
static int mv88e6xxx_g3_wait_bit(struct mv88e6xxx_chip *chip, int reg,
				 int bit, int val)
{
	int addr = chip->info->global3_addr;

	return mv88e6xxx_wait_bit(chip, addr, reg, BIT(bit), val);
}
...
{
	return mv88e6xxx_g3_wait_bit(chip, GLOBAL3_TCAM_OP,
				     GLOBAL3_TCAM_OP_BUSY, 0);
}
