...
static int mv88e6xxx_port_write(struct mv88e6xxx_chip *chip, int port,
				int reg, u16 val)
{
	int addr = mv88e6xxx_reg_port(chip, port);
				  ...
				  struct phy_device *phydev)
{
	...
	ret = mv88e6xxx_port_read(chip, port, PORT_PCS_CTRL);
	if (ret < 0)
		goto out;
...
static int _mv88e6xxx_stats_snapshot(struct mv88e6xxx_chip *chip, int port)
{
	...
	if (!mv88e6xxx_has(chip, MV88E6XXX_FLAG_STATS_HISTOGRAM))
		reg |= GLOBAL_STATS_OP_HIST_RX_TX;

	ret = _mv88e6xxx_reg_write(chip, REG_GLOBAL, GLOBAL_STATS_OP, reg);
	if (ret < 0)
		return ret;
			       ...
			       struct mv88e6xxx_hw_stat *stat)
{
	...
	case BANK0:
		return true;
	case BANK1:
		return mv88e6xxx_has(chip, MV88E6XXX_FLAG_STATS_BANK1_9) ||
			mv88e6xxx_has(chip, MV88E6XXX_FLAG_STATS_BANK1_10);
					    ...
					    int port)
{
	...
	switch (s->type) {
	case PORT:
		ret = mv88e6xxx_port_read(chip, port, reg);
		if (ret < 0)
			return UINT64_MAX;

		low = ret;
		if (s->sizeof_stat == 4) {
			ret = mv88e6xxx_port_read(chip, port, reg + 1);
			if (ret < 0)
				return UINT64_MAX;
			high = ret;
		}
		break;
	case BANK1:
		if (mv88e6xxx_has(chip, MV88E6XXX_FLAG_STATS_BANK1_9))
			ret |= GLOBAL_STATS_OP_BANK_1_BIT_9;
		if (mv88e6xxx_has(chip, MV88E6XXX_FLAG_STATS_BANK1_10))
			ret |= GLOBAL_STATS_OP_BANK_1_BIT_10;
		/* Fall through */
	case BANK0:
			       ...
			       struct ethtool_regs *regs, void *_p)
{
	...
	for (i = 0; i < 32; i++) {
		...
		ret = mv88e6xxx_port_read(chip, port, i);
		if (ret >= 0)
			p[i] = ret;
	}
			     ...
			     struct ethtool_eee *e)
{
	...
	reg = mv88e6xxx_port_read(chip, port, PORT_STATUS);
	if (reg < 0)
		goto out;
				 ...
				 u8 state)
{
	...
	reg = mv88e6xxx_port_read(chip, port, PORT_CONTROL);
	if (reg < 0)
		return reg;
	...
	if (oldstate != state) {
		...
		ret = mv88e6xxx_port_write(chip, port, PORT_CONTROL, reg);
		if (ret)
			return ret;
...
static int _mv88e6xxx_port_based_vlan_map(struct mv88e6xxx_chip *chip, int port)
{
	...
	reg = mv88e6xxx_port_read(chip, port, PORT_BASE_VLAN);
	if (reg < 0)
		return reg;
				...
				u16 *new, u16 *old)
{
	...
	ret = mv88e6xxx_port_read(chip, port, PORT_DEFAULT_VLAN);
	if (ret < 0)
		return ret;
	...
	if (new) {
		...
		ret = mv88e6xxx_port_write(chip, port, PORT_DEFAULT_VLAN, ret);
		if (ret < 0)
			return ret;
			       ...
			       u16 *new, u16 *old)
{
	...
	ret = mv88e6xxx_port_read(chip, port, PORT_BASE_VLAN);
	if (ret < 0)
		return ret;
	...
	if (new) {
		...
		ret = mv88e6xxx_port_write(chip, port, PORT_BASE_VLAN, ret);
		if (ret < 0)
			return ret;
	}

	/* Port's default FID bits 11:4 are located in reg 0x05, offset 0 */
	ret = mv88e6xxx_port_read(chip, port, PORT_CONTROL_1);
	if (ret < 0)
		return ret;
	...
	if (new) {
		...
		ret = mv88e6xxx_port_write(chip, port, PORT_CONTROL_1, ret);
		if (ret < 0)
			return ret;
					 ...
					 bool vlan_filtering)
{
	...
	ret = mv88e6xxx_port_read(chip, port, PORT_CONTROL_2);
	if (ret < 0)
		goto unlock;
	...
	if (new != old) {
		...
		ret = mv88e6xxx_port_write(chip, port, PORT_CONTROL_2, ret);
		if (ret < 0)
			goto unlock;
...
static int mv88e6xxx_switch_reset(struct mv88e6xxx_chip *chip)
{
	...
	for (i = 0; i < chip->info->num_ports; i++) {
		ret = mv88e6xxx_port_read(chip, i, PORT_CONTROL);
		if (ret < 0)
			return ret;

		ret = mv88e6xxx_port_write(chip, i, PORT_CONTROL, ret & 0xfffc);
		if (ret)
			return ret;
	}
...
static int mv88e6xxx_setup_port(struct mv88e6xxx_chip *chip, int port)
{
	...
	if (mv88e6xxx_has(chip, MV88E6XXX_FLAG_PORT_FORCE)) {
		 ...
		 * full duplex.
		 */
		reg = mv88e6xxx_port_read(chip, port, PORT_PCS_CTRL);
		if (dsa_is_cpu_port(ds, port) || dsa_is_dsa_port(ds, port)) {
	...
	if (mv88e6xxx_has(chip, MV88E6XXX_FLAGS_SERDES)) {
		ret = mv88e6xxx_port_read(chip, port, PORT_STATUS);
		if (ret < 0)
			return ret;
	...
	if (reg) {
		ret = mv88e6xxx_port_write(chip, port, PORT_CONTROL_2, reg);
		if (ret)
			return ret;
	}
	...
	ret = mv88e6xxx_port_write(chip, port, PORT_ASSOC_VECTOR, reg);
	if (ret)
		return ret;

	/* Egress rate control 2: disable egress rate control. */
	ret = mv88e6xxx_port_write(chip, port, PORT_RATE_CONTROL_2, 0x0000);
	if (ret)
		return ret;
	...
	if (mv88e6xxx_6352_family(chip) || mv88e6xxx_6351_family(chip) ||
		 ...
		 */
		ret = mv88e6xxx_port_write(chip, port, PORT_PAUSE_CTRL, 0x0000);
		if (ret)
			return ret;
	 ...
	 */
	ret = mv88e6xxx_port_write(chip, port, PORT_CONTROL_1, 0x0000);
	if (ret)
		return ret;
	 ...
	 */
	ret = mv88e6xxx_port_write(chip, port, PORT_DEFAULT_VLAN, 0x0000);
	if (ret)
		return ret;
...
static int mv88e6xxx_g1_setup(struct mv88e6xxx_chip *chip)
{
	...
	err = _mv88e6xxx_reg_write(chip, REG_GLOBAL, GLOBAL_CONTROL_2, reg);
	if (err)
		return err;
...
static int mv88e6xxx_detect(struct mv88e6xxx_chip *chip)
{
	...
	mutex_lock(&chip->reg_lock);
	id = mv88e6xxx_port_read(chip, 0, PORT_SWITCH_ID);
	mutex_unlock(&chip->reg_lock);
	if (id < 0)
		return err;
