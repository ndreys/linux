/*
 * Marvell 88e6xxx Ethernet switch TCAM support
 *
...
static int mv88e6xxx_read_g3(struct mv88e6xxx_chip *chip, int reg, u16 *data)
{
	return mv88e6xxx_read(chip, REG_GLOBAL3, reg, data);
}

static int mv88e6xxx_write_g3(struct mv88e6xxx_chip *chip, int reg, u16 data)
{
	return mv88e6xxx_write(chip, REG_GLOBAL3, reg, data);
}

static int mv88e6xxx_tcam_set_page0_data(struct mv88e6xxx_chip *chip,
					 u16 page0[32])
{
	int i, err;

	for (i = 2; i < 0x1c; i++) {
		err = mv88e6xxx_write_g3(chip, i, page0[i]);
		if (err)
			return err;
	}
...
static int mv88e6xxx_tcam_set_page1_data(struct mv88e6xxx_chip *chip,
					 u16 page1[32])
{
	int i, err;

	for (i = 2; i < 0x1c; i++) {
		err = mv88e6xxx_write_g3(chip, i, page1[i]);
		if (err)
			return err;
	}
...
static int mv88e6xxx_tcam_set_page2_data(struct mv88e6xxx_chip *chip,
					 u16 page2[32])
{
	int i, err, end;
	...
	for (i = 2; i < end; i++) {
		err = mv88e6xxx_write_g3(chip, i, page2[i]);
		if (err)
			return err;
	}

	/* Debug Port */
	return mv88e6xxx_write_g3(chip, GLOBAL3_P2_DEBUG28,
				  page2[GLOBAL3_P2_DEBUG28]);
}

static int mv88e6xxx_tcam_get_page0_data(struct mv88e6xxx_chip *chip,
					 u16 *page0)
{
	int i, err;

	for (i = 2; i < 0x1c; i++) {
		err = mv88e6xxx_read_g3(chip, i, &page0[i]);
		if (err)
			return err;
...
static int mv88e6xxx_tcam_get_page1_data(struct mv88e6xxx_chip *chip,
					 u16 *page1)
{
	int i, err;

	for (i = 2; i < 0x1c; i++) {
		err = mv88e6xxx_read_g3(chip, i, &page1[i]);
		if (err)
			return err;
	}
...
static int mv88e6xxx_tcam_get_page2_data(struct mv88e6xxx_chip *chip,
					 u16 *page2)
{
	int i, err;

	for (i = 2; i < 6; i++) {
		err = mv88e6xxx_read_g3(chip, i, &page2[i]);
		if (err)
			return err;
	}

	err = mv88e6xxx_read_g3(chip, GLOBAL3_P2_DEBUG28,
				&page2[GLOBAL3_P2_DEBUG28]);
	if (err)
		return err;

	return mv88e6xxx_read_g3(chip, GLOBAL3_P2_DEBUG31,
				 &page2[GLOBAL3_P2_DEBUG31]);
}

static int mv88e6xxx_tcam_wait(struct mv88e6xxx_chip *chip)
{
	return mv88e6xxx_wait(chip, REG_GLOBAL3, GLOBAL3_TCAM_OP,
			      GLOBAL3_TCAM_OP_BUSY);
}

/**
 * mv88e6xxx_tcam_set_mode() - Set the TCAM mode for a port
 * @chip: Switch chip to manipulate
 * @port: Set the TCAM mode for this port
 * @mode: Mode to be set.
 *
 * Set the TCAM mode for a port. The mode should be one of
 *	PORT_PRI_OVERRIDE_TCAM_DISABLE
 *	PORT_PRI_OVERRIDE_TCAM_48_BYTES
 *	PORT_PRI_OVERRIDE_TCAM_96_BYTES
 *
 * PORT_PRI_OVERRIDE_TCAM_48_BYTES and PORT_PRI_OVERRIDE_TCAM_96_BYTES
 * select how much of the frame the TCAM should perform comparisons
 * on.
 *
 * Return: Error code, or 0
 */
static int mv88e6xxx_tcam_set_mode(struct mv88e6xxx_chip *chip, int port,
				   int mode)
{
	u16 port_control;
	int err;
	...
	/* Read the current port state, and then disable the port
	 * before changing the TCAM mode.
	 */
	err = mv88e6xxx_port_read(chip, port, PORT_CONTROL, &port_control);
	if (err)
		goto out;
	...
	err = mv88e6xxx_port_write(chip, port, PORT_CONTROL, port_control);
out:
...
/**
 * mv88e6xxx_tcam_port_enable() - Enable the TCAM on a port
 * @chip: Switch chip to manipulate
 * @port: Set the TCAM mode for this port
 *
 * Return: Error code, or 0
...
{
	return mv88e6xxx_tcam_set_mode(chip, port,
				       PORT_PRI_OVERRIDE_TCAM_48_BYTES);
}

/**
 * mv88e6xxx_tcam_port_enable() - Disable the TCAM on a port
 * @chip: Switch chip to manipulate
 * @port: Set the TCAM mode for this port
 *
 * Return: Error code, or 0
...
/**
 * mv88e6xxx_tcam_flush_all() - Flush all TCAMs from the device
 * @chip: Switch chip to manipulate
 *
...
int mv88e6xxx_tcam_flush_all(struct mv88e6xxx_chip *chip)
{
	...
	err = mv88e6xxx_tcam_wait(chip);
	if (err)
		goto out;

	err = mv88e6xxx_write_g3(chip, GLOBAL3_TCAM_OP,
...
/**
 * mv88e6xxx_tcam_flush_entry() - Flush a single TCAM entry
 * @chip: Switch chip to manipulate
...
/**
 * mv88e6xxx_tcam_load_entry() - Load a single TCAM entry
 * @chip: Switch chip to manipulate
			      ...
			      struct mv88e6xxx_tcam_data *data)
{
	...
	err = mv88e6xxx_tcam_set_page2_data(chip, data->page2);
	if (err)
		goto out;
	...
	err = mv88e6xxx_tcam_set_page1_data(chip, data->page1);
	if (err)
		goto out;
	...
	err = mv88e6xxx_tcam_set_page0_data(chip, data->page0);
	if (err)
		goto out;
...
/**
 * mv88e6xxx_tcam_read() - Read a TCAM entry
 * @chip: Switch chip to manipulate
 * entry: Entry to be loaded
			...
			struct mv88e6xxx_tcam_data *data)
{
	...
	err = mv88e6xxx_tcam_get_page0_data(chip, data->page0);
	if (err)
		goto out;
	...
	err = mv88e6xxx_tcam_get_page1_data(chip, data->page1);
	if (err)
		goto out;
	...
	err = mv88e6xxx_tcam_get_page2_data(chip, data->page2);
out:
	mutex_unlock(&chip->reg_lock);
...
/**
 * mv88e6xxx_tcam_get_next() - Get the next valid TCAM entry
 * @chip: Switch chip to manipulate
 * entry: Entry to be loaded
 * @data: Data read from entry
 *
			    ...
			    struct mv88e6xxx_tcam_data *data)
{
	...
	err = mv88e6xxx_read_g3(chip, GLOBAL3_TCAM_OP, &reg);
	if (err)
		goto out;
	...
	*entry = reg & 0xff;
	...
	if (*entry != 0xff)
		return mv88e6xxx_tcam_read(chip, *entry, data);
...
/**
 * mv88e6xxx_tcam_set_match() - Set an octet of TCAM match data
 * @chip: Switch chip to manipulate
...
/**
 * mv88e6xxx_tcam_get_match() - Get an octet of TCAM match data
 * @chip: Switch chip to manipulate
...
/**
 * mv88e6xxx_tcam_set() - Set a parameter in the TCAM data structure
 * @chip: Switch chip to manipulate
 * @data: TCAM strucuture to set the parameter in
 ...
 * mv88e6xxx.h. These contain the correct bit shifts. In some cases,
...
int mv88e6xxx_tcam_set(struct mv88e6xxx_chip *chip,
		       struct mv88e6xxx_tcam_data *data,
		       enum mv88e6xxx_tcam_param param,
		       u16 value)
{
	u16 new_value;
	int err = 0;
...
/**
 * mv88e6xxx_tcam_get() - Get a parameter from the TCAM data structure
 * @chip: Switch chip to manipulate
