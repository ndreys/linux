...
#define MX_HW_FLOWCTRL_HW		0x2
#define MX_HW_FLOWCTRL_NO_CHANGE	0x3
...
static void mxuport_msr_event(struct usb_serial_port *port,
			      struct mxuport_port *mxport, u8 buf[4])
{
...
static int mxuport_set_rts(struct usb_serial_port *port, int state)
{
	struct mxuport_port *mxport = usb_get_serial_port_data(port);
	struct usb_serial *serial = port->serial;
	int err;
	...
	switch (state) {
	case MX_HW_FLOWCTRL_RTS_DISABLE:
		mxport->mcr_state &= ~UART_MCR_RTS;
		break;
	case MX_HW_FLOWCTRL_RTS_ENABLE:
		mxport->mcr_state |= UART_MCR_RTS;
		break;
	case MX_HW_FLOWCTRL_HW:
		/*
	...
	default:
		 ...
		 * MX_HW_FLOWCTRL_NO_CHANGE, which is not valid.
		 */
		err = -EINVAL;
		goto out;
	}
				    ...
				    struct usb_serial *serial)
{
	...
	if (I_IXOFF(tty) || I_IXON(tty)) {
		...
		dev_dbg(&port->dev, "%s - XON = 0x%2x, XOFF = 0x%2x\n",
			__func__, xon, xoff);
	} else {
		enable = 0;
	}
	...
	rts = MX_HW_FLOWCTRL_NO_CHANGE;

	/* H/W flow control settings */
	if (!old_termios ||
	    C_CRTSCTS(tty) != (old_termios->c_cflag & CRTSCTS)) {
		if (C_CRTSCTS(tty))
			rts = MX_HW_FLOWCTRL_HW;
		else
			rts = MX_HW_FLOWCTRL_RTS_ENABLE;
	}
	...
	if (C_BAUD(tty)) {
		...
		/* Drop DTR and RTS */
		rts = MX_HW_FLOWCTRL_RTS_DISABLE;
		mxuport_set_dtr(port, 0);
	}

	if (rts != MX_HW_FLOWCTRL_NO_CHANGE)
