...
static void rave_sp_wdt_load_reset_reason(struct watchdog_device *wdd)
{
	u8 cmd[] = {
		[0] = RAVE_SP_CMD_RESET_REASON,
		[1] = 0,
	};
	if (__rave_sp_wdt_exec(wdd, cmd, sizeof(cmd), &wdd->bootstatus, 1))
...
const static struct rave_sp_wdt_variant rave_sp_wdt_legacy = {
	.max_timeout = 255,
	.min_timeout = 1,
...
const static struct rave_sp_wdt_variant rave_sp_wdt_rdu = {
	.max_timeout = 180,
	.min_timeout = 60,
...
const static struct of_device_id rave_sp_wdt_variants[] = {
...
static int rave_sp_wdt_probe(struct platform_device *pdev)
{
	...
	rave_sp_wdt_load_reset_reason(wdd);
