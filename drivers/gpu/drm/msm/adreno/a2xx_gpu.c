...
static bool a2xx_me_init(struct msm_gpu *gpu)
{
	...
	/* XXX on a225 this should be 0x80000300 */
...
/* a2xx_hw_init doesn't work well with pm_suspend/pm_resume
 * wrapper to only call resume once..
 */
int a2xx_pm_resume(struct msm_gpu *gpu)
{
	struct adreno_gpu *adreno_gpu = to_adreno_gpu(gpu);
	struct a2xx_gpu *a2xx_gpu = to_a2xx_gpu(adreno_gpu);
	int ret = 0;

	if (!a2xx_gpu->pm_enabled) {
		ret = msm_gpu_pm_resume(gpu);
		if (!ret)
			a2xx_gpu->pm_enabled = true;
	}
	return ret;
}

int a2xx_pm_suspend(struct msm_gpu *gpu)
{
	return 0;
}
