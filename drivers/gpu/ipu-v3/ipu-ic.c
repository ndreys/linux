...
static int __ipu_ic_task_init(struct ipu_ic *ic, int in_width, int in_height,
			      int out_width, int out_height,
			      enum ipu_color_space in_cs,
			      enum ipu_color_space out_cs, u32 rsc)
{
	struct ipu_ic_priv *priv = ic->priv;
...
int ipu_ic_task_init(struct ipu_ic *ic, int in_width, int in_height,
		     int out_width, int out_height, enum ipu_color_space in_cs,
		     enum ipu_color_space out_cs)
{
	return __ipu_ic_task_init(ic, in_width, in_height, out_width,
...
static struct image_convert_ctx *ipu_image_convert_next(struct ipu_ic *ic)
{
	 ...
	 */
	if (!priv->ipu->prg_priv) {
		ipu_cpmem_set_axi_id(ch_in, 1);
		ipu_cpmem_set_axi_id(ch_out, 1);
	}
	...
	ret = __ipu_ic_task_init(ic, in->rect.width, in_height,
			out->rect.width, out->rect.height,
...
struct image_convert_ctx *ipu_image_convert_prepare2(struct ipu_soc *ipu,
...
EXPORT_SYMBOL_GPL(ipu_image_convert_prepare2);
...
static int ipu_image_convert_init2(struct device *dev, struct ipu_soc *ipu,
		struct ipu_ic_priv *priv)
{
	int ret;
		...
		unsigned long base, unsigned long tpmem_base)
{
	...
	ipu_image_convert_init2(dev, ipu, priv);
