...
{
	evm_set_key(priv->output, (size_t)priv->payload_size);
}
...
static int __init blob_gen_init(void)
{
	...
	priv->modifier = devm_kzalloc(&pdev->dev,
				      sizeof(*priv->modifier)*KEYMOD_LENGTH,
				      GFP_KERNEL | GFP_DMA);
	if (!priv->modifier) {
		ret = -ENOMEM;
		goto out_pdev_put;
	...
	priv->desc = devm_kzalloc(&pdev->dev, sizeof(*priv->desc),
				  GFP_KERNEL | GFP_DMA);
	if (!priv->desc) {
		ret = -ENOMEM;
		goto out_pdev_put;
